...
local function playCamerasForLocation(location, token)
    ...
    CreateThread(function()
        ...
        -- For non-smooth locations, showLocation() calls DoScreenFadeIn()
        ...
        -- the first within-location camera transition.
        if not smooth then
        ...
        for i = 2, #cameras do
            ...
            -- FIX: In the non-smooth path, activeCam = nextCam is set before the
            -- final Wait(FADE_BETWEEN_MS). A concurrent playCamerasForLocation call
            -- (auto-timer or rapid click) can run destroyActiveCam during that wait,
            -- destroying nextCam. Calling DestroyCam unconditionally on the now-
            -- destroyed handle is a native crash. DoesCamExist guards against this.
            if camToken ~= token then
                if DoesCamExist(nextCam) then
                    DestroyCam(nextCam, false)
                end
                return
            end
